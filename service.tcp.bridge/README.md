...
3. Kodi → Add-ons → My add-ons → Services → Kodi TCP Bridge →
   Configure to change the port, bind address, feedback rate, or the
   on-screen connect notification.
...
A pre-built SIMPL+ module (`KodiTCPBridgeFeedback.usp` / `KodiTCPBridgeFeedback.ush`) is provided in the [`Crestron/`](../Crestron/) folder:

- **Socket Interface**: Connect the Crestron **TCP/IP Client** `rx$` directly to the module's `From_Device$` buffer input (internal buffering reassembles split TCP packets automatically, so no Serial Gather is required).
...
## SIMPL side (Manual / Native Symbols)

If not using the SIMPL+ module:
- **TCP/IP Client** symbol, address = Kodi box, port = 9091, `Connect` held high.
- Send with **Serial Send** symbols: `UP\x0D`, `PLAY\x0D`, and so on.
- Receive into a **Serial Gather** with delimiter `\x0D`, then a
  **Serial I/O** or a few **Analog/Serial Buffer + Compare** blocks to split
  `KEY=VALUE` at the `=`.
- `PROGRESS` is already scaled 0–65535, so it drops straight onto an analog
  gauge join with no maths.
- If the processor's TCP client does not detect a dropped link quickly enough,
  set the heartbeat to 10–30 seconds and watch for `HEARTBEAT` in SIMPL.

## Building the zip

`build.ps1` and `build.bat` live in the repo root, one level above this
folder, and find the add-on themselves. Run the script from anywhere, or
double-click `build.bat`:

    .\build.ps1                    # writes dist\<id>-<version>.zip
    .\build.ps1 -OutDir C:\temp    # somewhere else
    .\build.ps1 -Clean             # wipe dist first

It reads the id and version out of `addon.xml`, so the zip name and internal
folder always track the manifest. Bump `version` in `addon.xml` and rebuild;
nothing else needs editing. Python on PATH is optional - if present, every
`.py` file is syntax-checked before packaging and the build aborts on error.
...
- A Kodi notification naming the client IP appears when a control system
  connects, which makes it obvious whether the processor is actually linked.
  Turn it off in the add-on settings if it gets in the way during playback.
  A refused connection (client limit reached) shows a warning toast instead.
